...
- **宝箱**七档，门槛 7 / 14 / 21 / 28 / 35 / 42 / 49 分。达标了就能开，也可以直接花星尘买。箱子发下来先躺着，孩子自己点开才抽内容，里面是什么开的那一刻才定。每一档到底给什么、随机件多大机会，有一张单独的表，从宝箱页进去看。
...
- **发布**顶部三格：写任务（能选挂大厅还是派给哪个孩子，派给谁是一排按钮；还能给任务配图，配图走游戏任务那一套）、写校准（后果怎么落地，按选的处理方式自己定档；修复类型也是一排按钮，最后一个「自己写一条」才展开输入框）、我发出的 N。
