...
登录是头像墙，点自己的头像再输四位数字。剩下的截图（任务大厅、券包、图鉴、成长报告、我的、怎么玩、登录页，以及家长端的审核、发布、家庭和账号、我的、规则说明）在 [`screenshots/`](screenshots/) 里。

## 孩子端

底栏五格：首页、任务、宝箱、券包、我的。

- **首页**是一张七分矩阵，七个维度横着走，这一周七天竖着走。拿到的那格填上它自己的颜色，今天套一圈橙框。抬头写「本周获得 X 能量，丢失 N 能量」（丢的只算家长打过分的那些天），底下两句是这一周现算的结论，右边挂着进成长报告的入口。有娱乐券在用的话，最上面是一条倒计时，写清到几点结束。心愿那一行上面是「遇到困难」，卡住了说一声就行。二级页从哪一屏进去的就退回哪一屏。
- **任务**分两块。大厅里是家长挂出来的活，孩子自己接；「我手上」是接了的和交了等确认的，两种都列出来，不会交了就不见。再往下一块是「任务记录」，最近三条直接摆在页面上，做完的、被退回的、自己放下的都在，想看全部点一下展开半屏，最多回溯近一个月。
- **宝箱**七档，门槛 7 / 14 / 21 / 28 / 35 / 42 / 49 分。达标了就能开，也可以直接花星尘买。箱子发下来先躺着，孩子自己点开才抽内容，里面是什么开的那一刻才定。每一档到底给什么、随机件多大机会，有一张单独的表，从宝箱页进去看。七档下面那根进度条按每档 7 分走，28 分就走到第 4 格、走到 42 分就只差一格。
...
- **我的**是等级和升级进度、星尘余额、特权道具、这个月的完美日，往下依次是心愿屋、成长报告、家庭、图鉴（收了多少张卡）、怎么玩（把那套规矩讲给他听的四页），最后是头像与皮肤、修改密码、退出登录。
...
- **打分**是四个孩子 × 七个维度，每天四种状态：还没打、可以补、已经打了、过期锁死。默认七项全给满，点掉他没做到的那几项，按「提交」才算数；交过一次那颗按钮变「修改」，点开是「提交」「取消」两颗并排。日期条在上面，先挑哪天再挑哪几项。
- **发布**顶部三格：写任务（能选挂大厅还是派给哪个孩子，派给谁是一排按钮；还能给任务配图，点开是底部半屏弹窗，走游戏任务那一套）、写校准（两张卡，第一张填是谁、哪件事，第二张选怎么处理；后果怎么落地，按选的处理方式自己定档；修复类型也是一排按钮，最后一个「自己写一条」才展开输入框）、我发出的 N。
- **我的**分四块：身份卡；我的（我的记录、改我的密码）；家里的事（家庭和账号、宝箱与道具、规则说明、心愿与许愿池、设置）；最下面单独一行备份与导出。家庭和账号是一页里把全家能量、许愿池、成员与账号摆在一起，改账号名、重置密码、停用都在成员那一行上。设置是独立一页，分两级：先按八组列，点进去才看见这一组的项，「通知与推送」「假期日历」也收在里面。版本号和项目地址在「我的」页脚。
- **规则说明**是家长这端的口径全书，三页：目录、打分·周期·结算、券·卡·有效期。孩子那端「怎么玩」讲的是同一套规矩，换成了他能自己读的说法。两边写的数字都从当前设置现取，你改过价它自己跟着变。
