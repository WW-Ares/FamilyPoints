...
判据：
  ...
  7. 前端文案在位    web/app.js 设置页里「家庭积分 v…」那句还在
...
# ---- 7. 前端文案在位 -------------------------------------------------------
# 版本号的落点换过一次：「我的」页原来那段一百八十字的「这一版改了什么」撤了，
# 版本号挪到设置页（那一页才是这个 App 的「关于」）最底下，跟仓库地址同一行。
# 这里认的是「界面上还看得见跑的是哪一版」，所以锚点跟着挪。
js = io.open(APPJS, encoding="utf-8").read()
mj = re.search(r"家庭积分\s*v['\"]?\s*\+\s*esc\(S\.data\.version", js)
if not mj:
    fail("web/app.js 里找不到版本号文案（设置页那句「家庭积分 v…」）")
else:
    ok("web/app.js 版本号文案在位")
