...
- 「收工」= 收到「完成 / 开始推送」那个信号（节奏见第二节），不是每轮改完就提交。收工时必须 `git commit`，不许把一堆改动丢在工作区过夜。
...
- 在里程碑点打标签。当前对外版本号看 `app/seed_data.py` 的 `APP_VERSION`（落后于代码里的版本号时以代码为准），对外第一版是 `v1.2`，此前的 `v3`–`v39` 内部编号已作废，只作沿革留在 `旧版历史备份/`。
- 这个仓库是**公开**的。提交前扫一眼别把真实数据、密钥、本机路径带进来（`app/data/`、`.workbuddy/`、`旧版历史备份/` 都已 gitignore）。

## 二、确认完成之后，才做自查 + 测试跑通

**改动的节奏**（WW先生 2026-09-23 定）：收到需求 → 规划 / 出模型 → 确认方案 → 开始改动 → 提出修改 → 确认方案 / 开始改动 → …… → 收到「完成 / 开始推送」→ 才跑验证并发版推送。

改动的过程中**不跑测试、不 commit、不 push**。每轮改完只交付「改了什么 + 实机截图」，等下一轮修改意见。收到收口的话之后，把下面这些跑通才算完成，不许「看着没问题」就交差。
